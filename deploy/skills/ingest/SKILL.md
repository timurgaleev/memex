...
2. **For each entity detected:**
   - `search` for the name -- does a page already exist?
   - **If yes:** load context with `page_get`. Use the compiled truth to
   ...
   - **If no:** assess notability (see `skills/_brain-filing-rules.md`). If the entity
     is worth tracking, create a new page with `page_put` (`<type>/<slug>`) and populate
     with what you know.
3. **After creating or updating pages:** nothing more to do — pages written via
   `page_put` are chunked and indexed by the server automatically. Embedding
   backfill and graph derivation run in the server's background cycle.
...
**Process:**
1. Fetch content (web fetch or equivalent)
...
2. **Save raw transcript** (both JSON and human-readable TXT) via `put_raw_data`
...
**Use the `put_raw_data` tool**, keyed to the page slug the raw content belongs
to. It stores raw text, transcripts, API responses, and metadata (JSON or text,
not binary) directly in the brain, so provenance survives page rewrites.

- **Text, transcripts, JSON payloads**: store via `put_raw_data` with the page
  slug and a type label (`transcript`, `article`, `api-response`, ...)
- **Large binaries** (video, audio, image files): the brain does not store
  binaries. Record the original URL or file path on the page's `**Source:**`
  line so the artifact stays reachable; store the extracted text/transcript
  via `put_raw_data`.

**Accessing stored raw data:**
- `get_raw_data` -- retrieve a stored raw payload by page slug
- `get_ingest_log` -- review what was ingested and when
...
4. **Only then: bulk execute** with throttling, checkpointing every 5-10 items.
...
Timeline entries: N
Raw source: [stored via put_raw_data / original at URL or path]
```

## Tools Used

- Read a page (page_get)
- Store/update a page (page_put)
- Add a timeline entry (add_timeline_event)
- Link entities (link)
- List tags for a page (get_tags)
- Tag a page (add_tag)
- Store raw data (put_raw_data)
