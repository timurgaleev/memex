...
> **Convention:** see `conventions/quality.md` (via `get_skill conventions/quality`) for
> citation rules, quote fidelity, and source-backed claims.
>
> **Boundary:** takes and facts are different layers — holder-attributed takes
> (beliefs, bets, hunches by a named holder) versus the brain owner's own hot
...
- A single-idea scope is preserved. Broad corpus or "map my concepts" prompts
  route to `skills/concept-synthesis` instead.
...
4. Check `page_list` for concept pages when the idea has an obvious concept
   slug or title.
...
- Full pages via `page_get` for the top relevant concept, note, transcript,
...
- Save the lineage as a brain page (e.g. `page_put` under `reports/`).
...
- Using real names, companies, funds, or deployment-specific examples in
  bundled fixtures or documentation.

## Related Skills and Operations

- `skills/concept-synthesis` - broad mutating concept-map synthesis.
- `skills/query` - general brain search and cited answers.
- `skills/brain-ops` - source attribution and brain-first behavior.
...
- `page_get` - full context for candidate source pages.
- `page_list` - concept-page discovery and scoped page enumeration.
