...
### With a native question tool (agent harnesses that ship one)

Some agent harnesses have a built-in question tool (e.g. Claude Code's
`AskUserQuestion`) that presents choices natively:
...
1. **Import via OAuth** — agent holds tokens for the session
2. **Import from Google Takeout export** — offline, from file
3. **Import a curated subset** — you pick who gets a page
...
people/jane-smith and there's a deal page at deals/acme-corp.

1. **Merge into Jane's page** — add to her timeline
2. **Add to Acme deal page** — this was primarily a deal discussion
3. **New meeting page** — standalone at meetings/2026-01-15-jane-acme
