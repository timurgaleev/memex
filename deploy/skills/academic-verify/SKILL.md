...
> for citation rules; every verdict cites the source data, not just the
...
The output is a brain page at `concepts/<claim-slug>` (written via
`page_put`) that records the claim, the trace, and the verdict — so
future references to the same claim can re-use the verified analysis.
...
- Not generic web research — use the agent's web search tooling directly
  for open-ended topic exploration.
- Not a brain-only lookup — that's the `query` tool.

## How it works (thin orchestration over the agent's web search)

academic-verify is a thin orchestrator. The actual web lookup is done by
the agent's own web search tooling (WebSearch / WebFetch or equivalent).
academic-verify's job is the *workflow*: scoping the claim precisely,
running a citation-focused research pass, then formatting the response
...
Step 2: Brain-first lookup
  query "<paper title> OR <author name> OR <claim keywords>"
  If the brain has prior verification of this claim, reuse it.

Step 3: Run a citation-mode web research pass
  Send the claim + brain context through the agent's web search tooling
  with a prompt that explicitly asks for:
...
Step 4: Format the verdict
  Write the result via page_put to concepts/<claim-slug>. The verdict is one of:
...
Step 5: Cross-link to original sources
  ...
  one if notable. Back-link with the `link` tool. Iron Law per
...
## Useful databases (the agent reaches these via its web search tooling)
...
- ❌ Skipping the brain-first lookup. Re-doing verification we've
  already done is wasted research spend.
- ❌ Skipping the web lookup and inventing the trace. The citations
  from the research pass are the evidence — without them, the verdict
  is just opinion.
