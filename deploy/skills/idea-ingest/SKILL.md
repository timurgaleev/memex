...
- Raw source preserved for provenance via the `put_raw_data` tool
...
2. **Preserve the raw source.** Store the fetched content for provenance with `put_raw_data`, keyed to the page slug it belongs to. Text and JSON go into the brain directly; for large binaries, record the original URL or file path on the page instead.

3. **Identify the author — MANDATORY people page.** Anyone whose thinking is worth ingesting is worth tracking.
   - `search` the brain for an existing author page
   - If no page → CREATE ONE (`page_put`) with compiled truth + timeline format
   - If page exists → update the timeline with this new publication (`add_timeline_event`)
   - Cross-link both directions (`link`)
