...
version: 0.33.0
description: |
  Lift a proven skill from a host workspace (e.g. your agent's project repo)
  back into the memex skillpack so other clients can scaffold it. Editorial
  workflow: the CLI does the file copy + privacy lint; this skill drives the
  judgment-heavy genericization (scrub real names, generalize triggers,
  lift workspace-specific conventions to references).
triggers:
  - "harvest this skill"
  - "harvest my skill"
  - "publish this skill to the skillpack"
  - "lift this skill"
  - "share this skill"
  - "promote this skill"
  - "promote my skill"
  - "skill upstream"
  - "into the skillpack core"
  - "skillpack bundle"
mutating: true
writes_pages: false
writes_to:
...
> file placement rules. This skill writes into the skillpack's own tree, not
> brain pages.
...
2. The editorial genericization in Phase 3 has scrubbed every
   workspace-specific reference (names, real entities, internal channels).
3. `memex skillpack harvest --dry-run` previewed the file set.
4. The real `memex skillpack harvest <slug> --from <host>` succeeded
   with `status: harvested` (no privacy-lint hits).
5. `memex skillpack check` passes on the new
   `skills/<slug>/SKILL.md`.
6. The user has reviewed the diff in the pack checkout and explicitly
   approved the commit.

If any of these is incomplete, the skill is NOT yet harvested — the
files may sit in the pack's working tree, but they're not landed.
...
- **Skipping the dry-run.** Always preview first. Files land in
  the pack's working tree; cleanup is a `git checkout` away, but you
  shouldn't need to.
...
- **Harvesting a skill that's still in flux.** Wait until the host
  ...
  then re-harvest, and that churns the pack for no benefit.
...
- The user developed a skill in a host workspace (a project repo, a
  personal agent fork, etc.) and wants other clients of the brain to
  be able to use it
...
2. **The skill is generalizable.** Strip-test in your head: replace
   every workspace-specific name. Does it still make sense as a skill?

3. **The user owns the pack checkout.** The harvest writes into
   the pack's working tree. They'll review and commit. Don't harvest
...
- What slug should the harvested skill have? (Slugs must be kebab-case,
  globally unique in the pack.)
- Which host repo is the source? (Path to repo root, not to the skill
  directory — e.g. `~/git/my-workspace`, not `~/git/my-workspace/skills/foo`.)
...
```bash
memex skillpack harvest <slug> --from <host-repo-root> --dry-run
```

The output shows:
- Which files would land in the pack's tree
...
1. **Workspace-specific names → generic phrasing**
   - Your workspace's pet name (`jarvis`, `hal9k`, whatever the fork
     is called) → `your agent workspace` (or `your deployment`)
   - Personal first names (`alice`, `bob`, etc.) → `the user` /
...
2. **Real entities → placeholders**
   ...
   - Internal chat channels → `#some-channel` or strip
   - Specific tracker IDs / ticket numbers → strip

3. **Workspace-specific conventions → references**
   - Mentions of `<host-repo>/docs/...` files → either lift the doc
     into the pack OR replace with a generic placeholder explanation
   - Mentions of `<host-repo>/skills/<other-workspace-only-skill>` →
     either decide to harvest that one too, or replace with a generic
     pattern reference

4. **Triggers array generalizes**
   - Read every entry in frontmatter `triggers:`. None should
     reference the user's name, workspace name, or internal tools.
   - "Have alice sign off on it" → "have the user sign off on it"
...
```bash
memex skillpack harvest <slug> --from <host-repo-root>
```

Default behavior:
- Path-confinement + symlink rejection at file copy
- Privacy linter runs against the gitignored local overlay
  `scripts/lib/harvest-private-patterns.local.txt` (plus built-in
  defaults: personal names, email, chat channels)
- On any match → rollback (delete the harvested files) + exit non-zero

Outcomes:
- `harvested` — success, files in the pack's tree
- `lint_failed` — privacy linter caught something. Go back to Phase 3,
  scrub the host file, retry.
- `slug_collision` — the pack already has a skill at that slug. Either
  use a different slug, or pass `--overwrite-local` if you really
  mean to replace.

### Phase 5 — Verify in the pack

After a successful harvest:

1. `memex skillpack check` — confirms the new SKILL.md meets the
   frontmatter contract.
2. `memex skillpack check --strict` — confirms no drift between the
   bundle and the pack's own checkout.
3. `memex skillpack list` — confirms the slug shows up in the bundle.
4. Review the diff: `cd <pack-root> && git diff -- skills/<slug>/`
5. Commit the additions in the pack (do NOT commit any leftover files
   in the host repo — harvest is a copy, not a move).

### Phase 6 — Downstream announcement (optional)

If other clients of the brain should pick up the new skill:
...
```bash
memex skillpack harvest <slug> --from <host-repo-root> --no-lint
```

**Document the bypass in the commit message.** Future maintainers
should be able to see WHY the lint was bypassed (e.g. "the flagged
name appears in a citation, not a real reference — verified manually").
...
- It does NOT publish to npm or a remote bundle. It writes to
  the pack's working tree; the user commits + ships via the normal
  release process.
...
- The pack's `skills/<slug>/` — every file in the host skill dir
  (copy)
- The pack's mirror path for declared paired sources
  (e.g. `src/commands/<slug>.ts` if the host SKILL.md declares it
  in frontmatter)

It does not touch any pack-level index file: enumeration comes from
`list_skills`, and `memex skillpack` builds the bundle's `.manifest.json`
from whatever directories are on disk at bundle time.
