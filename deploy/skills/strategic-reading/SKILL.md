...
writes_to:
  - concepts/
  - projects/
# The skill works on uploaded source text + writes brain pages; the only
# external call is the agent's own fetch for article inputs. Declarative
# opt-out from the brain-first analyzer.
brain_first: exempt
---
...
> for citation rules (every recommendation cites the source) and back-link
> enforcement.
>
> **Convention:** see `_brain-filing-rules.md` (via `get_skill _brain-filing-rules`)
> — output pages by primary subject (concepts/ for general strategy, projects/
...
Phase 1: Ingest the source
  ...
  ├── Article: fetch it with the agent's own web-fetch tooling
...
Phase 5: Write and deliver
  ...
  │     • If problem-specific: projects/<slug>/playbook
  │     • If general strategy: concepts/<slug>
  ├── page_put the page; link it to related concept/project pages.
