...
- **judge** — rule judges are deterministic and free; `llm` judges run on the
  utility tier (Haiku) by default, synthesis tier when the check needs nuance.
...
- **Bundled skills require explicit opt-in AND an independent held-out set.**
  ...
  in place the user passes BOTH `--allow-mutate-bundled` AND `--held-out <path>`
  with at least 5 benchmark-disjoint tasks; without the held-out set the run
  hard-refuses (exit 2). Drop `--allow-mutate-bundled` (or pass `--no-mutate`,
  the default for the background-cycle phase) to write proposed.md for review
...
1. **Generate the starter.** Run:
   ```
   memex eval skillopt X --bootstrap-from-skill
   ```
...
4. **Run the optimizer with `--split 1:1:1`:**
   ```
   memex eval skillopt X --bootstrap-reviewed --split 1:1:1
   ```
...
sequencing, privacy, refusal boundaries, or page filing — for those, hand-add
...
| Situation | Action |
|---|---|
| Skill has no benchmark | `memex eval skillopt foo --bootstrap-from-skill` → review + strengthen the judges → delete sentinel → `memex eval skillopt foo --bootstrap-reviewed --split 1:1:1` (see section above) |
| Skill has a `routing-eval.jsonl` and you want a head start | `memex eval skillopt foo --bootstrap-from-routing` → review the generated tasks → `--bootstrap-reviewed` (routing tasks test dispatch; tighten them into quality tasks before trusting) |
| Iterating on an existing skill | `memex eval skillopt foo --benchmark skills/foo/skillopt-benchmark.jsonl` |
| Costly run, want preview | Add `--dry-run` |
| Bundled skill (ships in the core skillpack) | Default writes proposed.md; to commit in place add `--allow-mutate-bundled` AND `--held-out <path>` (>=5 benchmark-disjoint tasks) — else it hard-refuses |
...
| Mid-run crash | `memex eval skillopt foo --resume <run-id>` |
...
- `skills/<name>/skillopt/audit-YYYY-Www.jsonl` — ISO-week-rotated audit trail
...
- **Don't optimize bundled skills without `--allow-mutate-bundled` AND
  `--held-out`.** They ship in the core skillpack and are load-bearing for
  downstream agents. In-place mutation requires both flags (held-out >=5
  benchmark-disjoint tasks); without the held-out set the run hard-refuses and
  points you at proposed.md.
...
- `conventions/quality.md` (via get_skill conventions/quality) — output quality standards skillopt enforces via judges
