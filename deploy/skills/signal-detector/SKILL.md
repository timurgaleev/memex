...
- Runs in parallel (spawned as the agent's own sub-agent, never blocks the main response)
...
3. Format: `- **YYYY-MM-DD** | Referenced in [page title](slug) — brief context`
...
3. For new FACTS with specific dates → call `add_timeline_event` on the entity's slug with the date and a one-line summary

**Auto-link:** When the brain's gazetteer auto-link is enabled, writing an
originals or ideas page that references a person or company creates the link
from the new page to that entity automatically on `page_put`. When it is off
(the default), call `link` explicitly for each entity reference. Timeline
entries always need explicit `add_timeline_event` calls.
...
- `page_get` — load existing entity pages
