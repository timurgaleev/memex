...
- **Note corpus:** `[Source: memory:{slug}]`
...
FROM that entity's page TO the page mentioning them. Add the typed edge with
`link`; append the human-readable entry with `page_append`.

Format: `- **YYYY-MM-DD** | Referenced in [page title](type/slug) -- context`
