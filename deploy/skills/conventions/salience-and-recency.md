# Salience + Recency on brain queries

YOU ARE IN CHARGE of the `salience` and `recency` knobs on the brain's
`query` op. They are TWO ORTHOGONAL axes — use either, both, or neither.

If you OMIT a parameter, the server auto-detects from query text via a
...
- `salience` — **mattering**. Boosts pages with high emotional weight
...
- `recency` — **age**. Boosts pages with recent effective date. NO
  mattering signal. Per-prefix decay (`concepts/`, `originals/`,
  `writing/` are evergreen; `daily/`, `media/x/`, `conversations/` decay
...
You can override at any time. The heuristic is smart but not infallible.
You have context the server doesn't.
...
The auto-detect heuristic is English-only. Non-English queries fall
through to the default `off` for both axes. Pass `salience` and
`recency` explicitly for non-English queries.
...
Independent of the axes. Filter to pages whose effective date is
within a range:

- `since: '7d'` — last 7 days (relative forms like `2w`, `1y` also work)
