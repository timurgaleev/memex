 ...
 *
 * Architecture guard (non-negotiable): this phase READS from `documents` +
 * `chunks` (the authored vault) and WRITES exclusively to `synth_atoms`. It
 * never touches `documents`, `chunks`, or the `pages` graph. Source notes are
 * sacrosanct.
 *
 * Safety properties:
  ...
  for (const item of parsed) {
    ...
    const concepts = Array.isArray(o.concepts)
      ? o.concepts
          ...
          .slice(0, 5)
      : [];
    out.push({ title, atom_type, body, concepts });
      ...
      try {
        await engine.query(
          `INSERT INTO synth_atoms
             (atom_key, source_ref, source_kind, source_hash, title, body, atom_type, concepts, model_id)
           VALUES ($1, $2, 'document', $3, $4, $5, $6, $7::jsonb, $8)
           ON CONFLICT (atom_key) DO NOTHING`,
          [key, doc.id, doc.contentHash16, atom.title, atom.body, atom.atom_type, JSON.stringify(atom.concepts), modelId],
        );
