...
/**
 * The held-out gate. Null unless the run finished: a run cut short by the cap
 * has compared the variants over fewer repeats than asked, which is no basis
 * for accepting an edit.
 */
export function heldoutGate(result: EvalResult, epsilon: number): GateResult | null {
  if (result.stopReason !== "end") return null;
  const baseline = summarize(result.rollouts, "baseline", "heldout").perRepeat;
  const candidate = summarize(result.rollouts, "candidate", "heldout").perRepeat;
