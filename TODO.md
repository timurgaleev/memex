...
A 64-agent comparison across nine subsystems; every claim was re-verified by
...
**Status: 14 shipped, 3 deferred by review, 18 open.**

### Shipped

- **[TS-01]** bash grammar wasm is an ABI-14 legacy-dylink blob whose libc imports the pinned runtime does not export — every real .sh file throws — v1.109.0
- **[TS-02]** CONCRETE FIX — exact packages, versions and commands (verified end-to-end) — v1.109.0
- **[TS-04]** No fallback when a grammar fails to load or parse — the file is dropped from the index entirely — v1.110.0
- **[QI-02]** Doc/description-level steering: `search` is not advertised as incomplete for concept questions — v1.110.0
- **[CLI-2]** Boolean flags swallow the following positional as their value — `memex embed --dry-run <slug>` performs a real embed — v1.110.0
- **[CLI-3]** `--key=value` inline form is unsupported and silently ignored — v1.110.0
- **[FCT-01]** Extractor cannot distinguish malformed model output from a genuinely empty turn — v1.110.0
- **[FCT-02]** No fallback JSON-recovery strategy when the model wraps its object in prose — v1.110.0
- **[FCT-03]** Malformed fact candidates are silently coerced into real fact rows instead of dropped or counted — v1.110.0
- **[AUTH-2]** No pre-auth rate limit on POST /mcp — two DB token lookups run before the limiter — v1.111.0
- **[AUTH-4]** rescopeClient cannot set or clear bound_slug_prefixes — a write fence can only be applied at registration — v1.111.0
...
- **[MCP-04]** No MCP ToolAnnotations — clients cannot distinguish read-only from destructive tools
  - Why deferred: dropped in review: the only safe version weakens the frozen tool_defs snapshot equality that guards the wire contract
