...
- [ ] **Admin surface (`/admin`) — IN PROGRESS (operator: "totally the same as
  ...
  - **B — admin SPA (frontend) — SCOPED, needs a focused frontend session.** The
    ...
    SPA reads reference shapes (`stats`/`health`/SSE `/admin/events`/`oauth_clients`
    ...
    - **B1 — scaffold + Login + Dashboard:** `admin/` Vite project (package.json,
      ...
      `revokeGrant`), App.tsx sidebar, Login.tsx (rename Reference→memex), Dashboard
...
- [x] `LINK_EXTRACTOR` bare-wikilink + verb-context resolution — bare-wikilink
  ...
  - [x] **verb-context live wiring — DONE (v1.29.0).** `syncVerbLinksForPage`
    ...
    verb-context item → full reference link-extraction parity.**
