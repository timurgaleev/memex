...
`<SECRETS_PREFIX>/google-calendar` Secrets Manager entry, opens a
browser for user consent, captures the auth code, exchanges it for a
refresh token, and writes the refresh_token back into the same secret.
...
Usage (from your laptop, NOT from the EC2 — needs a browser):
    ...
    AWS_REGION=<region> AWS_PROFILE=<your-profile> \\
      python3 scripts/gcal-oauth-bootstrap.py

Or use the wrapper which handles the venv automatically:

    AWS_REGION=<region> AWS_PROFILE=<your-profile> \\
      ./scripts/gcal-oauth-bootstrap.sh
...
Required local IAM permissions on `<SECRETS_PREFIX>/google-calendar`:
  secretsmanager:GetSecretValue
  secretsmanager:PutSecretValue
The EC2 instance role intentionally does NOT have PutSecretValue on
this secret — bootstrap runs operator-side only.
"""
...
SECRET_ID = f"{SECRETS_PREFIX}/google-calendar"
